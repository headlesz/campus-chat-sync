...
import { supabase, isEduEmail, signUpWithOTP, signInWithOTP } from "@/lib/supabase";
...
const Auth = () => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (session) navigate("/onboarding/voice");
    };
    checkSession();
  }, [navigate]);
  ...
  const handleSendLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = isNewUser 
        ? await signUpWithOTP(email, { name, age: parseInt(age) })
      ...
      <div className="w-full max-w-md animate-fade-in">
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4">
            <img src="/logo.jpeg" alt="CampusMatch" className="w-16 h-16 rounded-2xl object-cover" />
          </div>
